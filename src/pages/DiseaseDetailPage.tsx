...
export default function DiseaseDetailPage() {
  ...
  return (
    <Layout>
      <div className="px-4 py-8 space-y-8 max-w-lg md:max-w-3xl lg:max-w-4xl mx-auto pb-32 md:px-6 lg:px-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {/* Header */}
            ...
            <div className="p-8 space-y-6">
              <div className="flex justify-between items-start">
                ...
                <Badge
                  variant="outline"
                  className={cn(
                    "px-3 py-1 rounded-xl font-bold text-[10px] uppercase tracking-wider",
                    getSeverityColor(),
                  )}
                >
                  {t(`severity.${disease.severity}`)}
            ...
            <section className="space-y-3">
              ...
              <div className="grid gap-2">
                {disease.symptoms.map((symptom, i) => (
                  <div
                    key={i}
                    className="flex gap-3 text-sm font-medium text-foreground/80 p-4 rounded-2xl bg-muted/20 border border-border/10"
                  >
            ...
            <section className="space-y-3">
              ...
              <div className="grid gap-2">
                {disease.treatment.map((step, i) => (
                  <div
                    key={i}
                    className="flex gap-4 text-sm font-medium text-foreground/80 p-4 rounded-2xl bg-primary/5 border border-primary/10"
                  >
            ...
            <section className="space-y-3">
              ...
              <div className="grid gap-2">
                {disease.prevention.map((tip, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 text-sm font-medium text-foreground/80 p-4 rounded-2xl bg-success/5 border border-success/10"
                  >
