...
export default function ScanPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!apiResponse.ok) {
        throw new Error(
          "AI analysis failed. Please make sure the model server is running.",
        );
      }
      ...
      if (!data.is_healthy && !foundDisease) {
        toast.info(
          `Detected: ${data.class_name.replace(/___/g, " ").replace(/_/g, " ")}`,
        );
      }
    } catch (error) {
  ...
  return (
    <Layout>
      <div className="px-4 py-8 space-y-8 max-w-lg md:max-w-3xl lg:max-w-4xl mx-auto pb-32 md:px-6 lg:px-8">
        {/* Header */}
